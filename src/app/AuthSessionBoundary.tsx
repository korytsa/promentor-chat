...
export default function AuthSessionBoundary({ children }: AuthSessionBoundaryProps) {
  ...
  if (isHydrating) {
    return (
      <PageForShell title={AUTH_SESSION_HYDRATING_TITLE} description={AUTH_SESSION_HYDRATING_TEXT}>
  ...
  return (
    <PageForShell title={AUTH_SESSION_ENDED_TITLE} description={AUTH_SESSION_ENDED_DESCRIPTION}>
